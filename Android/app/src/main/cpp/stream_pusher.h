...
public:
    ...
    bool pushFrame(const uint8_t* data, int size, bool isKeyFrame) {
        ...
        if (ret < 0) {
            char errbuf[AV_ERROR_MAX_STRING_SIZE];
            av_strerror(ret, errbuf, sizeof(errbuf));
            LOGE("写入第%lld帧失败: %s (错误码: %d, 大小:%d, 关键帧:%s)", 
                 m_frameCount + 1, errbuf, ret, size, isKeyFrame ? "是" : "否");
            av_packet_free(&packet);
            return false;
        }
        
        LOGI("第%lld帧写入成功", m_frameCount + 1);
        m_frameCount++;
        m_frameCountSecond++;
        ...
        if (diff  > 1000 * 1000 ) {
            ...
            LOGE("Time:%s, Frames:%lld, Target FPS:%d, Actual FPS:%.1f", 
                timeStr.c_str(),
                m_frameCount,
                m_targetFps,
                actual_fps);
